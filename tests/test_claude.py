...
class TestClaudeAgentAddMetadataHeader:
    ...
    def test_yaml_uses_hash_comments(self):
        ...
        assert result.startswith("# Generated by agent-manager (claude agent)\n")
        assert "# File: config.yaml" in result
...
class TestClaudeAgentInitialize:
    ...
    def test_creates_directory_if_not_exists(self, tmp_path):
        ...
        with patch("am_agent_claude.claude.query") as mock_query:
            ...
            with patch("am_agent_claude.claude.message"):
                with patch.object(agent, "get_scope_directory", return_value=test_dir):
                    agent._initialize()
...
class TestClaudeAgentUpdate:
    ...
    def test_update_calls_initialize(self, tmp_path):
        ...
        with patch.object(agent, "_initialize") as mock_init:
            with patch.object(agent, "merge_configurations"), patch("am_agent_claude.claude.message"):
                agent.update(config)
    ...
    def test_update_calls_merge_configurations(self, tmp_path):
        ...
        with patch.object(agent, "merge_configurations") as mock_merge:
            ...
            mock_merge.assert_called_once_with(config, None)
    ...
    def test_update_creates_and_merges(self, tmp_path):
        ...
        org_path.mkdir()
        (org_path / ".claude").mkdir()
        (org_path / ".claude" / "test.md").write_text("# Test")
...
class TestClaudeAgentHooksIntegration:
    ...
    def test_markdown_hook_applied_during_merge(self, tmp_path):
        ...
        org_path.mkdir()
        (org_path / ".claude").mkdir()
        (org_path / ".claude" / "test.md").write_text("Content with trailing spaces   \n\n")
    ...
    def test_cursorrules_hook_applied_during_merge(self, tmp_path):
        ...
        org_path.mkdir()
        (org_path / ".claude").mkdir()
        (org_path / ".claude" / ".cursorrules").write_text("Rules content")
        ...
        output_file = test_dir / ".cursorrules"
        assert output_file.exists()
    ...
    def test_cursorrules_concatenates_from_multiple_levels(self, tmp_path):
        ...
        (org_path / ".claude").mkdir()
        (team_path / ".claude").mkdir()
        (personal_path / ".claude").mkdir()

        (org_path / ".claude" / ".cursorrules").write_text("# Organization rules\n- Follow company standards\n")
        (team_path / ".claude" / ".cursorrules").write_text("# Team rules\n- Use team conventions\n")
        (personal_path / ".claude" / ".cursorrules").write_text("# Personal rules\n- My preferences\n")
    ...
    def test_metadata_hook_applied_during_merge(self, tmp_path):
        ...
        org_path.mkdir()
        (org_path / ".claude").mkdir()
        (org_path / ".claude" / "config.yaml").write_text("key: value")
        ...
        output_file = test_dir / "config.yaml"
        content = output_file.read_text()
...
class TestClaudeAgentEdgeCases:
    ...
    def test_handles_empty_sources_list(self):
        ...
        with contextlib.suppress(IndexError):
            agent._add_metadata_header(content, file_name, sources)
    ...
    def test_multiple_hierarchy_levels(self, tmp_path):
        ...
        (org_path / ".claude" / "config.md").write_text("# Org Config")
        (team_path / ".claude" / "config.md").write_text("# Team Config")
        ...
        output_file = test_dir / "config.md"
        content = output_file.read_text()
    ...
    def test_agent_directory_with_parents(self, tmp_path):
        ...
        agent = Agent()
        test_dir = tmp_path / "deeply" / "nested" / ".claude"
